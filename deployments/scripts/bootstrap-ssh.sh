...
usage() {
	printf '%s\n' 'usage: bootstrap-ssh.sh --target user@host --endpoint URL --token TOKEN --server-name NAME --environment ENV [--port 22] [--binary PATH]'
	exit 2
}
...
while [ "$#" -gt 0 ]; do
	case "$1" in
		--target|--endpoint|--token|--server-name|--environment|--port|--binary|--service)
			[ "$#" -ge 2 ] || usage
			case "$1" in
				--target) TARGET=$2 ;;
				--endpoint) ENDPOINT=$2 ;;
